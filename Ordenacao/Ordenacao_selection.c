...
void ordenar_nomes(LISTA* *l);// -> função que ordena os registros por nome
void ordenar_zona(LISTA* *l);// -> função que ordena os registros por zona
void ordenar_telefone(LISTA* *l);// -> função que ordena os registros por telefone
void ordenar_endereco(LISTA* *l);// -> função que ordena os registros por endereço
void ordenar_tipo(LISTA* *l);// -> função que ordena os registros por tipo de entidade
...
setlocale(LC_ALL,"portuguese");
	...
	int opcao, escolha=1; //-> opções dos menus
...
{
	ENTIDADE aux; //Auxiliar que guardará somento as informações da lista. Não guarda endereços
	LISTA *atual, *proximo; //Ponteiros que ajudam a deslocar a lista
	atual = *l; //Atual recebe a primeira posição da lista
	if (atual == NULL){ //Se atual estiver Null, a lista está vazia
		
		printf("Lista Vazia!\n");
		
	}else{ //Caso a listan não esteja vazia: 
	
		while(atual != NULL){//Percorre até encontrar o último
			
			proximo = atual->proximo; //-> coloca a varial de ponteiro na proxima posicao
			while(proximo != NULL) //Percorre até encontrar o último item do ponteiro proximo
			{
				if(strcmp (atual->entidade.nome, proximo->entidade.nome) > 0) //-> compara qual string é menor lexicograficamente.
				{	
...
void ordenar_zona(LISTA* *l)
{
	...
	if (atual == NULL){ //Se atual estiver Null, a lista está vazia
		...
		while(atual != NULL){//Percorre até encontrar o último
			...
			while(proximo != NULL) //Percorre até encontrar o último item do ponteiro proximo
			{
				if(strcmp (atual->entidade.zona, proximo->entidade.zona) > 0) //-> compara qual string é menor lexicograficamente.
				{	
					//troca os valores(dados da entidade entre as o ponteiro proximo e o atual)
					aux = atual->entidade;
					atual->entidade = proximo->entidade;
...
void ordenar_telefone(LISTA* *l)
{
	...
	if (atual == NULL){ //Se atual estiver Null, a lista está vazia
		...
		while(atual != NULL){//Percorre até encontrar o último
			...
			while(proximo != NULL) //Percorre até encontrar o último item do ponteiro proximo
			{
				if(strcmp (atual->entidade.telefone, proximo->entidade.telefone) > 0) //-> compara qual string é menor lexicograficamente.
				{	
					//troca os valores(dados da entidade entre as o ponteiro proximo e o atual)
					aux = atual->entidade;
					atual->entidade = proximo->entidade;
...
/************************************************ 
 * ordenar por endereços                        *
 * objetivo: Organiza a lista por endereço      *
 * entrada : Lista encadeada                    *
 * saida   : lista ordenada por endereço        *
 ************************************************/
void ordenar_endereco(LISTA* *l)
{
	...
	if (atual == NULL){ //Se atual estiver Null, a lista está vazia
		...
		while(atual != NULL){//Percorre até encontrar o último
			...
			while(proximo != NULL) //Percorre até encontrar o último item do ponteiro proximo
			{
				if(strcmp (atual->entidade.endereco, proximo->entidade.endereco) > 0) //-> compara qual string é menor lexicograficamente.
				{	
					//troca os valores(dados da entidade entre as o ponteiro proximo e o atual)
					aux = atual->entidade;
					atual->entidade = proximo->entidade;
...
void ordenar_tipo(LISTA* *l)
{
	...
	if (atual == NULL){ //Se atual estiver Null, a lista está vazia
		...
		while(atual != NULL){//Percorre até encontrar o último
			...
			while(proximo != NULL) //Percorre até encontrar o último item do ponteiro proximo
			{
				if(strcmp (atual->entidade.tipo, proximo->entidade.tipo) > 0) //-> compara qual string é menor lexicograficamente.
				{	
					//troca os valores(dados da entidade entre as o ponteiro proximo e o atual)
					aux = atual->entidade;
					atual->entidade = proximo->entidade;
